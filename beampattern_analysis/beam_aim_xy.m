...
addpath 'R:\Magdiel\Data\Preprocessing\Mic\Beam_Output'
...
% if exist('bat_type','var') && strcmp(bat_type,'rousettus')
  mic_proc_dir='R:\Magdiel\Data\Preprocessing\Mic\Beam_Output';
...
trials=dir([mic_proc_dir 'batV_1_20231108_093831_mic_data_bp_proc']);
% else
...
mic_data_dir='R:\Magdiel\Data\Preprocessing\Mic\Mic_Data_Detect';
...
for tt=1  %:length(trials)
  ...
  if save_movie
    ...
    v=VideoWriter(['R:\Magdiel\Data\Preprocessing\Mic\Beam_AV' out_fname '.avi'],'uncompressed avi');
...
% 07/06/2022
    %bpp.mic_loc(2,:)=[4.8817235231399535E-4	2.339397216796875	1.2550709228515624];%original
    % bpp.mic_loc(2,:)=[0	2.67671435546875 1.2458001708984374]; %30B
% 7/11/2022
    %bpp.mic_loc(2,:)=[-0.05505141448974609	2.35066943359375 1.2496038818359374];%original 7/11/2022 
    %bpp.mic_loc(2,:)=[-0.26754031372070314*2	2.279890380859375	1.24859375];%15F*2 = 30B
% 7/14/2022
    %bpp.mic_loc(2,:)=[-0.4308331604003906	3.06223779296875	1.2328836669921874];%original
    %bpp.mic_loc(2,:)=[-0.09134941101074219	2.466008056640625 1.2525828857421875];%15B
% 7/17/2022
    %bpp.mic_loc(2,:)=[-0.008367223739624024	2.3394423828125	1.2414189453125];%original
    %bpp.mic_loc(2,:)=[-0.00503903341293335	2.362529296875 1.54376220703125];%30U


% 7/27/2022
    %   bpp.mic_loc(2,:)=[-0.062210777282714846, 2.19740771484375, 1.25077490234375]; %original perch location 7/27/2022
% 8/02/2022
    % bpp.mic_loc(2,:)=[-0.08613481140136718	2.343202392578125 1.2410469970703124]; %original 8/02/2022
    % bpp.mic_loc(2,:)=[-0.10193054962158203	2.635473876953125	1.2365994873046875]; %30back 8/02/2022
    % bpp.mic_loc(2,:)=[-0.0969466552734375	2.354041015625	1.5365498046875]; %30up 8/02/2022
% 8/04/2022
    %bpp.mic_loc(2,:)=[-0.08613481140136718	2.343202392578125	1.2410469970703124]; %original
    %bpp.mic_loc(2,:)=[-0.10193054962158203	2.635473876953125	1.2365994873046875]; %30back
    %bpp.mic_loc(2,:)=[-0.0969466552734375	2.354041015625	1.5365498046875];%30U
% 11/16/2022
    %bpp.mic_loc(2,:)=[-0.08527720642089844	3.248646728515625   1.1995660400390624]; %30 back 11/16/2022
%     bpp.mic_loc(2,:)=[-0.09886363983154296	2.948463623046875 1.2058658447265624]; %original 11/16/2022
    %bpp.mic_loc(2,:)=[-0.09245469665527344	2.77369091796875	1.2105877685546875]; %15F

%11/30/2022
    %bpp.mic_loc(2,:)=[-0.08878011322021484	2.33650048828125	1.253489013671875]; %original 11/30/2022
    %bpp.mic_loc(2,:)=[-0.226201797485352	2.31217236328125 1.09460473632813]; %15D
    %bpp.mic_loc(2,:)=[-0.22235102844238303	2.4673342285156 1.23448974609375]; %15B
    %bpp.mic_loc(2,:)=[-0.209567260742188	2.00801501464844 1.23767810058594];%30F


% 12/08/22
    %bpp.mic_loc(2,:)=[-0.14521258544921875	2.328887451171875 1.2475595703125]; %original 12/08/22
%12/20/22
%     bpp.mic_loc(2,:)=[-0.14055842590332032	2.3196142578125 1.232498779296875]; %original 12/20/22
    %bpp.mic_loc(2,:)=[-0.15258158874511718	2.0074195556640624 1.235646240234375]; % 30 F 12/20/2022
%12/21/22
    %bpp.mic_loc(2,:)=[-0.13314645385742188	2.07233447265625	1.2393153076171874]; %original
    %bpp.mic_loc(2,:)=[-0.11631631469726562	2.072432861328125	1.092054931640625]; % 15D 12/21/2022
    %bpp.mic_loc(2,:)=[-0.1234656753540039	2.080036865234375	1.3928154296875]; %15U
    
%12/22/22  
    %bpp.mic_loc(2,:)=[-0.17667898559570314	2.31460205078125 1.23483349609375]; %original 12/22/2022
    % bpp.mic_loc(2,:)=[-0.02234539794921875	2.32388427734375 1.2340003662109376]; %15R 12/22/2022
    % bpp.mic_loc(2,:)=[-0.16639816284179687	2.32180517578125 0.9476177368164063]; %30D
   

%1/06/2023
%    bpp.mic_loc(2,:)=[-0.12165958404541016, 2.28627587890625, 1.242847412109375]; %original perch location 1/06/2023
    %bpp.mic_loc(2,:)=[-0.10053314208984375	2.2841142578125 1.392965087890625]; %15 U 1/06/2023
     %bpp.mic_loc(2,:)=[0.02919888687133789	2.282841064453125 1.242837158203125]; %15R 1/06/2023
%1/07/2023
    %bpp.mic_loc(2,:)=[-0.11080235290527343	2.280138916015625 1.2485592041015625]; %original 1/07/2023
    %bpp.mic_loc(2,:)=[-0.1274131622314453	2.57671435546875 1.2458001708984374]; %30B
    %bpp.mic_loc(2,:)=[-0.26754031372070314	2.279890380859375	1.24859375];%15F
%     bpp.mic_loc(2,:)=[-0.26754031372070314	2.279890380859375	1.24859375];%15L


%1/08/2023
%     bpp.mic_loc(2,:)=[-0.13326434326171874, 2.88758349609375, 1.205453857421875]; %original perch location 1/08/2023
    %bpp.mic_loc(2,:)=[-0.12797000885009766	3.04362158203125 1.20231640625]; %15B
    %bpp.mic_loc(2,:)=[-0.12380921936035157	2.730952392578125 1.209076171875]; %15F

%1/09/2023
    %bpp.mic_loc(2,:)=[-0.07581965637207032	2.280422119140625	1.2467335205078125]; %original

%1/10/2023 
    %bpp.mic_loc(2,:)=[-0.08446115875244141, 2.889064453125, 1.2118409423828125]; %original perch location 1/10/2023
%1/11/2023 
%     bpp.mic_loc(2,:)=[-0.1644509735107422, 2.89736669921875, 1.2168316650390625]; %original perch location 1/11/2023
    %bpp.mic_loc(2,:)=[-0.020903520584106446	2.8959775390625 1.2170472412109374]; %15R
%1/12/2023
    %bpp.mic_loc(2,:)=[-0.10488689422607422	2.8934912109375	1.21340087890625]; %original perch location
    %bpp.mic_loc(2,:)=[-0.10830216979980468	3.194203369140625	1.198256103515625]; %30B
%1/13/2023
%     bpp.mic_loc(2,:)=[-0.10244842529296876	2.524356689453125 1.251665771484375]; %original
    %bpp.mic_loc(2,:)=[0.04636844253540039	2.516872802734375	1.25144873046875]; %15R
    


%1/16/2023
    %bpp.mic_loc(2,:)=[-0.1677200164794922	2.4247626953125 1.2624990234375]; %15F
    %bpp.mic_loc(2,:)=[-0.1803780517578125	2.58340234375 1.2610059814453125]; %original
    %bpp.mic_loc(2,:)=[-0.18158709716796875	2.735484130859375 1.26140283203125]; %15B
    %bpp.mic_loc(2,:)=[-0.5120487060546876	3.31992578125 1.2361986083984375]; %15U

%1/17/2023
    %bpp.mic_loc(2,:)=[-0.10138038635253906	2.28451318359375	1.2462823486328125]; %original
    %bpp.mic_loc(2,:)=[0.21716429138183593	2.272991210937 1.2436995849609376]; %30R
    %bpp.mic_loc(2,:)=[-0.10015943908691406	2.43562646484375 1.2444620361328125]; %15B




%1/18/2023
    %bpp.mic_loc(2,:)=[-0.16744908142089843	2.90753759765625 1.195053466796875]; %original
    %bpp.mic_loc(2,:)=[-0.17461151123046875	2.906567626953125	1.0417469482421875]; %15D

%1/19/2023
%     bpp.mic_loc(2,:)=[-0.19081942749023437	2.26380859375	1.2363955078125]; %original
...
%1/20/2023 
    %  bpp.mic_loc(2,:)=[-0.1563638153076172, 2.271655517578125, 1.2348447265625]; %original perch location 1/20/2023

    
    
    % bpp.mic_loc(2,:)=[-0.110802352905273,	2.28013891601563,	1.24855920410156];   %original perch location  5
...
%       audiowrite(['Y:\Spatial Memory Project (Xiaoyan and Nikita)\beampattern_analysis\Working Code\proc_output\' trials(tt).name '.wav'],y./max(abs(y)),...
      audiowrite(['R:\Magdiel\Data\Preprocessing\Mic\Beam_Output' trials(tt).name '.wav'],y./max(abs(y)),...
    ...
    if ~exist(['..\animate_beam_dirs\' out_fname '.mp4'],'file')
      [~,~]=system(['C:\video_tools\ffmpeg_64\bin\ffmpeg -y ' ...
        '-i R:\Magdiel\Data\Preprocessing\Mic\Beam_Output' out_fname '.avi '...
        '-c:v libx264 -crf 20 -pix_fmt yuv420p ' ...
        '..\animate_beam_dirs\' out_fname '_no_audio.mp4']);
    end
    delete(['R:\Magdiel\Data\Preprocessing\Mic\Beam_Output' out_fname '.avi'])
  end
end
