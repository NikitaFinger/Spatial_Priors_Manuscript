...
if save_movie
    ...
    if exist(aud_fn, 'file')
        ...
        command = ['C:\video_tools\ffmpeg_git\bin\ffmpeg -y -i ' wav_file ' -c:a libfdk_aac -b:a 48k -cutoff 18000 ' aac_file];
        ...
        mp4_file_output = ['..\animate_beam_dirs\' out_fname '.mp4'];
        command = ['C:\video_tools\ffmpeg_64\bin\ffmpeg -y -i ' mp4_file_input ' -i ' aac_file ...
            ' -c:v libx264 -crf 20 -pix_fmt yuv420p -c:a copy -bsf:a aac_adtstoasc ' mp4_file_output];
        ...
        disp(['Audio file not found: ' aud_fn]);
    end

    % Only if audio isn't present - still create animation
    if ~exist(['..\animate_beam_dirs\' out_fname '.mp4'], 'file')
        mp4_file_input = ['Z:\Nikita\Prism_Experiment\Data\Preprocessing\Mic\Beam_AV\' out_fname '.mp4'];
        command = ['C:\video_tools\ffmpeg_64\bin\ffmpeg -y -i ' mp4_file_input ' -c:v libx264 -crf 20 -pix_fmt yuv420p ' ...
