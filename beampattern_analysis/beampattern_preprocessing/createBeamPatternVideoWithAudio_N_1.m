...
%         bpp.mic_loc(2,:) = [-1.74700744628906 1.09099462890625 1.33053503417969];
        ...
        for fr = frames(1):frames(end)
            ...
            % Find indices of good and bad calls
            good_call = find(bpp.proc.chk_good_call); % Indices of good calls
            bad_call = find(~bpp.proc.chk_good_call); % Indices of bad calls

            % Print the list of bad calls
            disp('Bad calls found:');
            disp(bad_call); % Print the list of bad calls

            % Existing logic for processing good calls
            call_present = ismember(call_track_locs(calls_with_track(good_call)), fr); % Check if any good call is present in the current frame

            call_indx_1 = find(call_present); % Find indices within call_present where true
            call_indx = good_call(call_indx_1); % Convert these indices back to original good call indices

            if ~isempty(call_indx) && ismember(fr, frames) % If there are good calls present and the current frame is within the frame range
                % Print details of the good calls present in the current frame
                % disp(['Frame: ', num2str(fr)]);
                % disp('Good calls in this frame:');
                % disp(call_indx);

                % Proceed with plotting the beam pattern
