    ...
    for i = 1:2:length(varargin)
        if strcmpi(varargin{i}, 'date_data')
            gui_op.date_data = varargin{i+1};
        end
    end
    ...
    disp(gui_op.date_data)
    [fname,pname] = uigetfile(strjoin({'*',gui_op.date_data,'*.mat'},''),'Pick processed file',pname);
    if isequal(pname,0)
        return
    end
    setpref('bp_check_gui','proc_file_path',pname)
    disp(['Loading processed file: ',fname]);
    data = load(fullfile(pname,fname));  % load all processed data (beampattern data)
    ...
    %gui_op.base_dir = fileparts(pname);
    ...
    if ~ispref('bp_check_gui','mic_data_path')
        md_path = fullfile(gui_op.base_dir,'/Mic_Data_Detect/');
    else
        md_path = getpref('bp_check_gui','mic_data_path');
    end
    if ~exist(fullfile(md_path,[data.files.proc_data,'.mat']),'file')  % if cannot find mic_data in path then select new one
        [md_fname,md_path] = uigetfile(strjoin({'*',gui_op.date_data,'*mic_data.mat'},''),'Select the mic data file',md_path);

        if isequal(md_path,0)
            return;
        end
    end
    data.files.mic_data = md_fname;
