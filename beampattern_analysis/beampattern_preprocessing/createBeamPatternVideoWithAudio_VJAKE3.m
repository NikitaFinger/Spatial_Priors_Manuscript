    ...
    for tt = 1  % :length(trials)
        ...
        for fr = frames(1):frames(end)
            ...
            if ~isempty(call_indx) && ismember(fr, frames)
                ...
                goodch1 = goodch(angle_notnanidx);
                ...
                if d2
                    ...
                    norm_I_mic_vec = I_mic_vec ./ max(call_dB - min(call_dB)) * .2;
        ...
        if save_movie
            ...
            enhanceGraphics(outputDir, out_fname, frames, bat, bpp, side_view, aa, vid_frate);
        end
    end
end

function enhanceGraphics(outputDir, out_fname, frames, bat, bpp, side_view, aa, vid_frate)
    ...
    % Video writer for the enhanced video
    v_enhanced = VideoWriter(fullfile(outputDir, [out_fname '_enhanced.mp4']), 'MPEG-4');
    v_enhanced.FrameRate = vid_frate;
    open(v_enhanced);

    % Plot and record frames for enhanced video
    for fr = frames(1):frames(end)
        plot3(bat(frames(1):fr, 1), bat(frames(1):fr, 2), bat(frames(1):fr, 3), ...
            '-', 'linewidth', 3, 'color', 'k');
        if side_view
            view(0, 0)
        else
        ...
        writeVideo(v_enhanced, getframe(gcf));
    end
