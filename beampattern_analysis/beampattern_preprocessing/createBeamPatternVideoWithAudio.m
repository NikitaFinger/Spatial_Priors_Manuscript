...
outputDir = 'Z:\Nikita\Prism_Experiment\Data\Preprocessing\Mic\Beam_AV\';
  ...
  if save_movie
    ...
    % v=VideoWriter(['/Volumes/pbs-mosslab/Magdiel/Data/Preprocessing/Mic/Beam_AV/batV' out_fname '.avi'],'uncompressed avi');
...
%     /bpp.mic_data.fs*bpp.track.fs) + 400;
  ...
  %add locations of the perch
 bpp.mic_loc(2,:)=[-1.74700744628906	1.09099462890625	1.33053503417969]; 
...
%     delete(pb);   %keep the bat fly path, xiaoyan
  ...
  if save_movie
    ...
    % Extract the trial identifier from the file name
    ...
    if exist(aud_fn, 'file')
        ...
        % Extract the relevant segment of the audio signal
        y = wav_data.sig(samp1:samp2, ch);

        % Write the cropped audio to a temporary file
        temp_audio_file = fullfile(tempdir, [trials(tt).name '.m4a']);
        audiowrite(temp_audio_file, y ./ max(abs(y)), round(wav_data.fs / (bpp.track.fs / vid_frate)));

        % Read the existing video file
        videoFile = [outputDir out_fname '.mp4'];
        video = VideoReader(videoFile);

        % Create a new video writer object with audio
        outputVideoFile = [outputDir out_fname '_with_audio.mp4'];
        ...
        audioReader = dsp.AudioFileReader(temp_audio_file);
        audioWriter = dsp.AudioFileWriter(outputVideoFile, 'FileFormat', 'MPEG4');
        while ~isDone(audioReader)
            audioData = audioReader();
            audioWriter(audioData);
        end
    ...
    % Delete the original video file if necessary
    if exist([outputDir out_fname '.mp4'], 'file')
        delete([outputDir out_fname '.mp4']);
    end
end
...
%       
%       %convert audio, you need to convert to ffmeg- to do this go here: 
%       [~,~]=system(['C:\video_tools\ffmpeg_git\bin\ffmpeg -y ' ...
%         '-i Z:\Nikita\Prism_Experiment\Data\Preprocessing\Mic\Beam_Output\' trials(tt).name '.wav ' ...
%         '-c:a libfdk_aac -b:a 48k -cutoff 18000 ' ...
%         'Z:\Nikita\Prism_Experiment\Data\Preprocessing\Mic\Beam_Output\' trials(tt).name '.aac ']);
%       delete(['Z:\Nikita\Prism_Experiment\Data\Preprocessing\Mic\Beam_Output\' trials(tt).name '.wav'])
%       
