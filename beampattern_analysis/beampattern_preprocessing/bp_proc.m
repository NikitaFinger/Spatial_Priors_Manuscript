function data = bp_proc(data,pname,chk_indiv_call,track_cut_idx,T,H,date_data,floor_mics)

%modified by Shivam on Mar 13 '23 for prism project use case, modified by
%NF Oct 2023 for use in the Prism Project
...
%if temp and humidity not specified assume defaults
if exist('T','var')
    data.param.tempC = T;
end
...
%if floor mics are not specified ignore
if exist('floor_mics','var')
    data.param.mic_floor_idx = floor_mics;
end

%if date is not specified ignore
...
data.path.bat_pos = '../../Data/Preprocessing/Vicon/Bat_Position/';
data.path.mic_data = '../../Data/Preprocessing/Mic/Mic_Data_Detect/';
data.path.mic_detect = '../../Data/Preprocessing/Mic/Mic_Data_Detect/';
data.path.mic_info = '../../Data/Preprocessing/Vicon/Mic_Position/';
data.path.mic_bp = '../../Data/Preprocessing/Mic/Mic_BP/';
data.path.mic_sens = '../../Data/Preprocessing/Mic/Mic_Sens/';
data.path.proc_output = '../../Data/Preprocessing/Mic/Beam_Output/';

% Load data and info
data.files.bat_pos = uigetfile(strjoin({'*',date_data,'*.mat'},''),"Select bat position file",fullfile(cd,data.path.bat_pos));
data.files.mic_data = uigetfile(strjoin({'*',date_data,'*data.mat'},''),"Select mic data file",fullfile(cd,data.path.mic_data));
data.files.mic_detect = regexprep(data.files.mic_data,'data','data_detect');  % xxx_detect.mat
data.files.mic_info = uigetfile(strjoin({'mic_info*',date_data,'*.mat'},''),"Select mic info file",fullfile(cd,data.path.mic_info));
data.files.mic_sens = uigetfile('*.mat',"Select mic sensitivity file",fullfile(cd,data.path.mic_sens));
data.files.mic_bp = uigetfile('*.mat',"Select mic bp file",fullfile(cd,data.path.mic_bp));
...
%save data
save_name = regexprep(data.files.mic_data,'.mat','_bp_proc.mat');
save(fullfile(data.path.proc_output,save_name),'-struct','data');
...
elseif length(bat.bat_pos)==3  % 3 markers on the head
    ...
    pos = pos(cut_idx,:,:);
    track = mean(pos,3,'omitnan');  % raw bat track: mean of three points
    track_t = -fliplr(0:size(track,1)-1)/data.track.fs;  % Time stamp of the track [sec]
